...
public class Drives extends GenericSubsystem{
	 ...
	 */
	private SolenoidIF shiftingSol;

	/**
	 * the solenoid used to engage and disengage the pto
	 */
	private SolenoidIF ptoSol;
	...
	protected boolean init() {
		...
		//RIGHT
		rightFront = WPI_Factory.getInstance().getCANTalon(IO.CAN_DRIVES_RIGHT_FRONT);
		rightBack = WPI_Factory.getInstance().getCANTalon(IO.CAN_DRIVES_RIGHT_BACK);
		//TODO:: real robot has it A, B.
		encoderRight = WPI_Factory.getInstance().getEncoder(IO.DIO_RIGHT_DRIVES_ENC_A,IO.DIO_RIGHT_DRIVES_ENC_B);
		//encoderRight = WPI_Factory.getInstance().getEncoder(IO.DIO_RIGHT_DRIVES_ENC_B,IO.DIO_RIGHT_DRIVES_ENC_A);
		encoderDataRight = WPI_Factory.getInstance().getEncoderData(encoderRight,DISTANCE_PER_TICK);
		//LEFT
		leftBack = WPI_Factory.getInstance().getCANTalon(IO.CAN_DRIVES_LEFT_BACK);
		leftBack.setInverted(true);
		leftFront = WPI_Factory.getInstance().getCANTalon(IO.CAN_DRIVES_LEFT_FRONT);
		leftFront.setInverted(true);
		//TODO:: same as right encoder
		//encoderLeft = WPI_Factory.getInstance().getEncoder(IO.DIO_LEFT_DRIVES_ENC_A,IO.DIO_LEFT_DRIVES_ENC_B);
		leftA = WPI_Factory.getInstance().getDigitalInput(IO.DIO_LEFT_DRIVES_ENC_A);
		leftB = WPI_Factory.getInstance().getDigitalInput(IO.DIO_LEFT_DRIVES_ENC_B);
		encoderLeft = WPI_Factory.getInstance().getEncoder(leftA, leftB);
		//encoderLeft = WPI_Factory.getInstance().getEncoder(IO.DIO_LEFT_DRIVES_ENC_B,IO.DIO_LEFT_DRIVES_ENC_A);
		encoderDataLeft = WPI_Factory.getInstance().getEncoderData(encoderLeft,-DISTANCE_PER_TICK);
		//OTHER
		angleGyro = WPI_Factory.getInstance().getAnalogGyro(IO.ANALOG_IN_ANGLE_GYRO);
		angleGyro.calibrate();
		wantedLeftPower = 0;
		wantedRightPower = 0;
		currentDriveState = DriveState.IN_LOW_GEAR;
		shiftingSol = WPI_Factory.getInstance().getSolenoid(IO.PNU_SHIFTER);
		ptoSol = WPI_Factory.getInstance().getSolenoid(IO.PNU_PTO);
		...
		tiltGyro = WPI_Factory.getInstance().getAnalogGyro(IO.ANALOG_IN_TILT_GYRO);
		tiltGyro.calibrate();
